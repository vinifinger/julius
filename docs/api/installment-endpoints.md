...
| `status` | `string` | ✅ | `PENDING`/`PAID` | Initial status |
...
| `paidInstallments` | `integer` | No | How many installments have been paid |
| `pendingInstallments`| `integer` | No | How many installments are pending |
| `paidAmount` | `number` | No | Sum of amounts for paid installments |
| `pendingAmount` | `number` | No | Sum of amounts for pending installments |

> **UI Tip:** Use `paidInstallments / totalInstallments` to render a progress bar.
